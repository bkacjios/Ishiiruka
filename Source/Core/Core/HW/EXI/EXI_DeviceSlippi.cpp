...
std::unordered_map<u8, std::string> CEXISlippi::getNetplayNames()
{
	...
	else if (NetPlay::netplay_client && NetPlay::netplay_client->IsConnected())
	{
    auto netplayPlayers = NetPlay::netplay_client->GetPlayers();
		for (auto it = netplayPlayers.begin(); it != netplayPlayers.end(); ++it)
		{
			auto player = *it;
      u8 portIndex = NetPlay::netplay_client->FindPlayerPad(player);
			if (portIndex < 0)
			{
				continue;
			}
...
void CEXISlippi::prepareOnlineMatchState()
{
	...
	u8 delay = (u8) 2; //SConfig::GetInstance().m_slippiOnlineDelay;
	if (slippi_netplay)
	{
...
void CEXISlippi::prepareFileLength(u8 *payload)
{
	...
	INFO_LOG(SLIPPI, "prepareFileLength LoadFile: %s", fileName.c_str());
...
void CEXISlippi::prepareFileLoad(u8 *payload)
{
	...
	INFO_LOG(SLIPPI, "prepareFileLoad LoadFile: %s", fileName.c_str());
...
void CEXISlippi::DMAWrite(u32 _uAddr, u32 _uSize)
{
	...
	if (memPtr == nullptr)
	{
		...
		Dolphin_Debugger::PrintCallstack(Common::Log::LOG_TYPE::SLIPPI, Common::Log::LOG_LEVELS::LNOTICE);
		m_read_queue.clear();
		return;
	}
