...
void CEXISlippi::prepareOnlineMatchState()
{
	...
	if (mmState == SlippiMatchmaking::ProcessState::CONNECTION_SUCCESS)
	{
		...
		{
#ifdef LOCAL_TESTING
			slippi_netplay = std::make_unique<SlippiNetplayClient>(false);
#else
	...
	if (localPlayerReady && remotePlayerReady)
	{
		...
		*stage = Common::swap16(stageId);
		...
		u32 *selection1 = (u32 *)&onlineMatchBlock[0x20];
		u32 *selection2 = (u32 *)&onlineMatchBlock[0x24];

		onlineMatchBlock[0xB] = (u8)SConfig::GetInstance().m_slippiItemFrequency;
		WARN_LOG(SLIPPI, "Item Frequency BYTE: 0x%x", onlineMatchBlock[0xB]);

		*selection1 = Common::swap32((u32)SConfig::GetInstance().m_slippiItemSelection1);
		*selection2 = Common::swap32((u32)SConfig::GetInstance().m_slippiItemSelection2);
		WARN_LOG(SLIPPI, "Item selection 1: 0x%x", *selection1);
		WARN_LOG(SLIPPI, "Item selection 2: 0x%x", *selection2);
	...
	u8 delay = (u8)SConfig::GetInstance().m_slippiOnlineDelay;
	if (slippi_netplay)
	{
		// Set the current match delay for the overlay
		slippi_netplay->currentMatchDelay = delay;
	}

	// Add delay frames to output
	m_read_queue.push_back(delay);
